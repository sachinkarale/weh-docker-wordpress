...
class WPSS_Security {
	...
	static public function early_post_intercept() {
		...
		if( rs_wpss_is_xmlrpc() ) {
			...
			$c['name'] = 'P_XMLRPC';
			if( !empty( $HTTP_RAW_POST_DATA ) && is_string( $HTTP_RAW_POST_DATA ) && FALSE !== strpos( stripslashes( $HTTP_RAW_POST_DATA ), '<methodName>pingback.ping</methodName>' ) ) { return; }
		}
		...
		/* TO DO: rs_wpss_bad_robot_blacklist_chk() */

		if( ( TRUE === WPSS_IP_BAN_ENABLE && rs_wpss_is_login_page() ) || rs_wpss_is_xmlrpc() || rs_wpss_is_doing_rest() ) {
			/* BAD ROBOT BLACKLIST */
